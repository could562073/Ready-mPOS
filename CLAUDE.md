...
**目前 = `2.3.0`**（分類軟刪除墓碑 + `_config` 加 `deleted` 欄 + 孤兒類別回收 → MINOR：新增向後相容的欄位與復原功能）。
...
│       │   ├── categories.ts          # 類別 localStorage CRUD + 軟刪除墓碑/孤兒回收（2.3.0）+ calcFees
