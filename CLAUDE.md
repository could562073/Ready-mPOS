...
│       │   ├── txSheets.ts            # 逐筆交易⇄Sheets 列轉換、新舊格式偵測、id 對帳（純函式，Phase 5）+ 金額/日期儲存格解析與 RowParse 辨識聯集（2.4.2 落地／2.5.0 接線）
...
- **儲存格值解析（2.4.2 落地／2.5.0 接線）**：`parseSheetAmount`／`parseSheetDate` 純函式，處理 `FORMATTED_VALUE` 的顯示字串（`"1,234"`、`"NT$1,234"`、`"(500)"`、`"2026/8/23"`）與 `UNFORMATTED_VALUE` 的真值（數字、日期序列號，序號 0 = 1899-12-30 UTC）。🔴 **解析失敗一律回 `null`，絕不回 0 或猜日期**；`parseSheetDate` 刻意拒收月日順序不明的 `8/23/2026`／`23/8/2026`。已於 2.5.0 接進 `rowToTx` 與 `parseOldMonthRows`。
- **`RowParse` 辨識聯集（2.5.0）**：`rowToTx` 回傳 `{kind:'tx';seed} | {kind:'skip'} | {kind:'unreadable'}`。🔴 **`skip`（整列空白，正常）與 `unreadable`（有內容但日期/金額/id 讀不出來）必須分開**——舊版兩者都回 `null` 被一視同仁略過，使用者手加的列於是先被忽略、再被整月 clear+覆蓋刪除。`unreadable` 會讓該月進 `unreadableMonths`，`planMonthsToRewrite` 在**最後一步**把這些月份排除（優先權高於不受門檻限制的 `upgradeMonths`），並經 `syncError`（`kind:'UNKNOWN'`，不觸發同步暫停）告知使用者。
- **讀寫型別（2.5.0）**：月份分頁寫入 `valueInputOption=RAW`、`_config` 讀取 `valueRenderOption=UNFORMATTED_VALUE`。⚠️ **備份匯出刻意維持 `FORMATTED_VALUE`**（給人看的復原檔，序號日期無法閱讀）；死碼 `pullAllFromSheets`／`syncMonthToSheets` 的 `USER_ENTERED` 刻意不動。
