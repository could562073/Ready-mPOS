...
// 導覽到「設定 → 支出類別」類別管理頁（reload 後重新進入用）
async function gotoExpenseCategories(page: Page) {
...
test.beforeEach(async ({ page }) => {
  await page.addInitScript(() => {
    ...
    //    測到 localStorage 持久化，這裡只在「尚未有值」時種子，避免覆蓋測試中途存入的二級。
...
test('二級分類：新增／改名／設預設／刪除 全流程並正確持久化', async ({ page }) => {
  ...
  await gotoExpenseCategories(page)
  ...
  // 重新載入整頁後再開啟同一類別，真正驗證 saveCategories→getCategories 的 localStorage round-trip
  await page.reload()
  await gotoExpenseCategories(page)
  await openCategoryEditor(page, '雜支')
  await expect(page.getByText('編輯類別')).toBeVisible()
  ...
  // 再次 reload 後開啟確認：只剩「水費」，且沒有預設殘留（刪除也持久化到 localStorage）
  await page.reload()
  await gotoExpenseCategories(page)
  await openCategoryEditor(page, '雜支')
  await expect(page.getByPlaceholder('二級名稱')).toHaveCount(1)
