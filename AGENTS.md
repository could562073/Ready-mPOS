...
- **第 2 次優化（進行中）**: 逐筆交易改造 — **Phase 1–5 完成**。Phase 1：`Transaction` 型別、Dexie v3 自動遷移、`explodeDailyRecord` 拆解純函式 + Vitest、交易 CRUD/hook。Phase 2：二級分類純函式 CRUD + `CategoryEditSheet` 管理 UI + E2E。Phase 3：二級經 Sheets `_config` 跨裝置同步（`serializeSubs`/`parseSubs`）+ 修 push/pull 資料流失 + feature 分支同步隔離到獨立測試試算表（🔴 併 main 前須改回正式名）。Phase 4：記帳改逐筆交易 — 「記帳」tab 換成 `LedgerPage`（單日列表 + 右下 FAB → `TransactionSheet` 記帳，選一級自動帶入 `defaultSubId`），寫入 `transactions` + Playwright E2E。Phase 5：逐筆交易雲端同步 — 月份分頁改為新格式（`日期|收支|一級|二級|金額|備註|id`，`lib/txSheets.ts` 純函式：`isNewTxFormat` 偵測、`txToRow`/`rowToTx`、`mergeTransactionsById` 以 id 去重對帳）；舊格式 pull 時就地 `explodeDailyRecord` 拆解並標記待改寫，改寫前必先 `backupSpreadsheet`（Drive 時間戳備份，`drive.file` scope，需重新授權），備份失敗則本輪跳過所有舊格式改寫；`syncAll`/`restoreFromSheets` 已切換讀寫 `db.transactions`。⚠️ **Dashboard / 月結仍讀舊 `DailyRecord`**，待 Phase 7；帳目月曆落地頁待 Phase 6。cutover（改回正式試算表名、遷移真實資料）為使用者核准的硬停，本期未執行，分支仍用 `AUTO_SHEET_NAME` 測試名——已知限制：cutover 首次同步時本機遷移交易與雲端 re-explode 交易的隨機 id 不同，`mergeTransactionsById` 會誤判為不同筆而重複，需先解決（見 spec）。分支 `feature/line-item-transactions-redesign`。設計 spec：`docs/superpowers/specs/2026-07-01-line-item-transactions-redesign-design.md`。
...
- ⚠️ **Dashboard / 月結仍讀舊 `DailyRecord` 模型**（待 Phase 7）；帳目月曆落地頁待 Phase 6。雲端同步已於 Phase 5 切換到 `transactions`（見下方）。
...
- ⚠️ **已知 cutover 限制**：cutover 首次同步時，本機已遷移的交易（`explodeDailyRecord` 產生隨機 id）與雲端 pull 時對同一批舊資料 re-explode 出的交易 id 不同，`mergeTransactionsById` 無法辨識為同一筆而造成重複——cutover 前需先擇一解法（走 `restoreFromSheets` 單方向覆蓋本機、或改用 deterministic id）。cutover（改回正式試算表名、對真實使用者資料執行遷移）為使用者核准的硬停，本期未執行，分支仍使用 `AUTO_SHEET_NAME` 測試試算表名。
